 ...
 * The SuperTurbostake Developers 2013
 */
...
bool ThreadSafeAskFee(int64_t nFeeRequired, const std::string& strCaption)
{
    if(!guiref)
        return false;
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. SuperTurbostake can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
// TODO: implement qtipcserver.cpp for Mac and Windows
    ...
    {
        if (strlen(argv[i]) >= 7 && strncasecmp(argv[i], "SuperTurbostake:", 7) == 0)
        {
    ...
    app.setOrganizationName("SuperTurbostake");
    app.setOrganizationDomain("SuperTurbostake.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("SuperTurbostake-Qt-testnet");
    else
        app.setApplicationName("SuperTurbostake-Qt");
