...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    loop
    {
        ...
        if (nQuantity > 0)
        {
            ...
            {
                int64_t nMoveToFee = min(nChange, MIN_TX_FEE - nPayFee);
                nChange -= nMoveToFee;
                nPayFee += nMoveToFee;
            }
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            itemOutput->setData(COLUMN_AMOUNT_int64_t, Qt::DisplayRole, out.tx->vout[out.i].nValue);
            ...
            // coin age
            int nDayWeight = (min((GetAdjustedTime() - out.tx->GetTxTime()), (int64_t) STAKE_MAX_AGE) - nStakeMinAge) / 86400;
            uint64_t coinAge = max(out.tx->vout[out.i].nValue * nDayWeight / COIN, (int64_t) 0);
            itemOutput->setData(COLUMN_COINAGE, Qt::DisplayRole, coinAge);

            // priority
            double dPriority = ((double)out.tx->vout[out.i].nValue  / (nInputSize + 78)) * (out.nDepth+1); // 78 = 2 * 34 + 10
            itemOutput->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPriority));
            itemOutput->setData(COLUMN_PRIORITY_int64_t, Qt::DisplayRole, (int64_t)dPriority);
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setData(COLUMN_AMOUNT_int64_t, Qt::DisplayRole, nSum);
            itemWalletAddress->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPrioritySum));
            itemWalletAddress->setData(COLUMN_PRIORITY_int64_t, Qt::DisplayRole, (int64_t)dPrioritySum);
        }
    }
