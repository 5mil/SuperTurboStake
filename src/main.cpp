...
// Copyright (c) 2015 The SuperTurbostake Developers
...
const string strMessageMagic = "SuperTurbostake Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // SuperTurbostake: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    //  (prevent mempool memory exhaustion attack)
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // SuperTurbostake: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // SuperTurbostake: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// SuperTurbostake: find block wanted by given orphan block
...
int64_t GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // SuperTurbostake: subsidy is cut in half every 16x multiply of difficulty
    ...
    while (bnLowerBound + CENT <= bnUpperBound)
    {
        ...
        if (fDebug && GetBoolArg("-printcreation"))
            printf("GetProofOfWorkReward() : lower=%"PRI64d" upper=%"PRI64d" mid=%"PRI64d"\n", bnLowerBound.getuint64_t(), bnUpperBound.getuint64_t(), bnMidValue.getuint64_t());
    ...
    int64_t nSubsidy = bnUpperBound.getuint64_t();
...
{
    static int64_t nRewardCoinYear = 5000 * CENT;  // creation amount per coin-year
...
static const int64_t nTargetTimespan = 1 * 24 * 60 * 60;  // one week
static const int64_t nTargetSpacingWorkMax = 12 * 20 * STAKE_TARGET_SPACING; // 2-hour
...
// SuperTurbostake: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // SuperTurbostake: target change every block
    // SuperTurbostake: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // SuperTurbostake: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // SuperTurbostake: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // SuperTurbostake: coin stake tx earns reward instead of paying fee
            uint64_t nCoinAge;
        ...
        else
        {
            ...
            // SuperTurbostake: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SuperTurbostake: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    int64_t nBIP16SwitchTime = fTestNet ? 1329264000 : 1333238400;
    ...
    // SuperTurbostake: track money supply and mint amount info
    ...
    // SuperTurbostake: fees are not collected by miners as in bitcoin
    // SuperTurbostake: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// SuperTurbostake: total coin age spent in transaction, in the unit of coin-days.
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    nCoinAge = bnCoinDay.getuint64_t();
    return true;
}

// SuperTurbostake: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // SuperTurbostake: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // SuperTurbostake: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // SuperTurbostake: record proof-of-stake hash value
    ...
    // SuperTurbostake: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // SuperTurbostake: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // SuperTurbostake: coinbase output should be empty if proof-of-stake block
    ...
    // SuperTurbostake: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // SuperTurbostake: check that the block satisfies synchronized checkpoint
    ...
    // SuperTurbostake: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SuperTurbostake: check proof-of-stake
    ...
    // SuperTurbostake: verify hash target and signature of coinstake tx
    ...
    // SuperTurbostake: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // SuperTurbostake: getblocks may not obtain the ancestor block rejected
    ...
    // SuperTurbostake: if responsible for sync-checkpoint send it
...
// SuperTurbostake: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64_t)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "SuperTurbostake", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "SuperTurbostake", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // SuperTurbostake: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // SuperTurbostake: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // SuperTurbostake: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // SuperTurbostake: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // SuperTurbostake: should not enter safe mode for longer invalid chain
    // SuperTurbostake: if sync-checkpoint is too old do not enter safe mode
    ...
    // SuperTurbostake: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // SuperTurbostake: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // SuperTurbostake: record my external IP reported by peer
        ...
        // SuperTurbostake: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // SuperTurbostake: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // SuperTurbostake: tell downloading node about the latest block if it's
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    static int64_t nTimeLastPrintMessageStart = 0;
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // SuperTurbostake: if coinstake available add coinstake tx
    ...
    int64_t nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // SuperTurbostake: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // SuperTurbostake: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
