...
// SuperTurbostake: optional setting to unlock wallet for block minting only;
...
// SuperTurbostake: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // SuperTurbostake: timestamp must not exceed spend time
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // SuperTurbostake: not avatar mode
                        {
...
// SuperTurbostake: create coin stake transaction
...
// SuperTurbostake: check 'spent' consistency between wallet and txindex
// SuperTurbostake: fix wallet spent state according to txindex
...
// SuperTurbostake: disable transaction (only for coinstake)
